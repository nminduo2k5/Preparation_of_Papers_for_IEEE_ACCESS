...
The rapid growth of digital technology in equity markets and the increasing availability of real-time market data  have created new opportunities for applying artificial intelligence (AI) in trading and investment analysis. In Vietnam’s stock market, this transformation is especially relevant. By the end of 2024, the market has experienced substantial growth in securities trading accounts, with the vast majority owned by domestic investors. Retail investors accounted for more than 80\% of total trading value \cite{vietnamnews2025}. These figures demonstrate that Vietnam’s market is heavily driven by retail investors and exhibits high volatility, structural inefficiencies, and an evolving regulatory framework.
...
Initial applications of Multi-Agent Systems in finance were primarily aimed at replicating market behavior rather than predicting prices. Groundbreaking research by LeBaron \cite{lebaron2006} and Tesfatsion \cite{tesfatsion2006} demonstrated that interacting agents can recreate typical market phenomena, such as high short-term volatility and market bubbles—patterns that standard statistical analysis or single-model learning methods fail to explain.
...
\begin{itemize}
    ...
    Uses the primary LSTM forecast combined with technical tools such as the \textbf{Relative Strength Index (RSI)}, \textbf{Moving Average Convergence Divergence (MACD)}, and \textbf{Bollinger Bands}. By analyzing trends and momentum, it refines the initial prediction to deliver a more accurate forecast along with a confidence rating for the technical analysis.

    \item \textbf{InvestmentExpert Agent:} 
    Evaluates critical financial metrics including the \textbf{Price-to-Earnings ratio (P/E)}, \textbf{Price-to-Book ratio (P/B)}, \textbf{Return on Equity (ROE)}, \textbf{Earnings Per Share growth (EPS)}, and \textbf{Debt-to-Equity ratio}. Through comprehensive fundamental analysis, it assesses the company's financial health, valuation attractiveness, and growth potential, generating an investment recommendation (\textbf{BUY, SELL, or HOLD}) accompanied by a confidence score reflecting the strength of its fundamental assessment.

    \item \textbf{RiskExpert Agent:} 
    Analyzes volatility profiles, market correlation matrices, and historical drawdown data. It evaluates overall portfolio risk through volatility modeling and correlation dynamics, providing a quantified risk level with corresponding position sizing guidance.

    \item \textbf{TickerNews Agent:} 
    Processes company-specific news sourced from platforms including CafeF, VietStock, and earnings releases. Employing sentiment classification and event-driven impact evaluation, it outputs a sentiment polarity rating coupled with an expected market impact magnitude.

    \item \textbf{MarketNews Agent:} 
    Examines macroeconomic updates, market sentiment data, and central bank communications. Through systemic risk evaluation and regime classification, it generates a contextual market rating reflecting prevailing macroeconomic conditions.

    \item \textbf{StockInfo Agent:} 
    Analyzes trading volume, liquidity trends, and technical metadata to assess market microstructure. The agent presents a liquidity-adjusted feasibility score followed by execution-related suggestions for optimal trade deployment.
\end{itemize}
...
\subsection{Conclusion}
Multi-Agent architectures—particularly \textbf{Hierarchical}—are conclusively established as optimal for Vietnam's emerging market. By embedding deliberative reasoning, cross-validation, and consensus-building mechanisms, MAS delivers superior accuracy, stability, and interpretability compared to Single-Agent LLMs and LSTM baselines. The framework's ability to synthesize heterogeneous information streams through coordinated agent interaction represents a paradigm shift from single-model forecasting to ensemble intelligence, making it ideally suited for retail-dominated, sentiment-driven markets where traditional approaches fail to capture complex behavioral and structural dynamics.

\section{Discussion}
\subsection{Key Findings and Implications}
Empirical evaluation reveals strong horizon-dependent architectural effectiveness. Ensemble Voting demonstrates consistent superiority in medium-term forecasting (14-day) across multiple stocks: FPT (5.7\%), VCB (4.0\%), VCS (1.6\%), DXG (10.0\% MAPE), validating ensemble learning theory~\cite{dietterich2000ensemble}. Short-term predictions (3-day) exhibit stock-specific architectural preferences with no single dominant architecture, suggesting idiosyncratic factors require tailored approaches~\cite{campbell1997stock}. Long-term predictions (60-day) show mixed preferences: Hierarchical for VCB (8.9\%), Ensemble for DXG (19.1\%) and FPT (4.6\%), Round Robin for VCS (3.4\%), reflecting increased uncertainty in extended horizons~\cite{guidolin2007asset}.
...
\textit{Ensemble Voting} consistently demonstrated superior performance: FPT (5.7\%), VCB (4.0\%), VCS (1.6\%), DXG (10.0\% MAPE), effectively integrating divergent technical and fundamental signals.
...
\textit{Multi-Agent Systems} (MAS) significantly outperform LSTM baselines and Single-Agent LLM configurations. LSTM models exhibited significant stochastic instability with pronounced amplitude distortion. Single-Agent architectures lacked robust error-correction under high volatility. Multi-Agent architectures—particularly \textit{Ensemble Voting}—acted as \textit{stability anchors} by aggregating heterogeneous perspectives, effectively filtering idiosyncratic noise in retail-driven markets.
